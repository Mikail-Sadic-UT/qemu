 ...
 * Derived from the real Huygens system VPD with all identifying fields
 * (serial numbers, part numbers, type-model, enclosure type, firmware
 * versions) replaced with space (0x20) or zero (0x00) placeholders.
 *
 ...
 *
 * ECC bytes were recomputed using the vpdecc_create_ecc() algorithm
 * (see scripts/recompute_vpd_ecc.c, sourced from vpdecc/ in openpower-vpd-parser).
 * Verified: vpdecc_check_data() returns VPD_ECC_OK for all records.
 *
...
static void huygens_bmc_i2c_init(AspeedMachineState *bmc)
{
    ...
    /* I2C8: System VPD at 0x53 (atmel,24c32 = 4 KiB per DTS; IPZ data is 2 KiB) */
