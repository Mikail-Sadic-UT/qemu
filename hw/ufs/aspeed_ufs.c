...
#define IS_UTMRCS           BIT(9)    /* UTP Task Management Request Completion */
...
/*
 * UTP Transfer Request Descriptor (UFSHCI spec §7.2.1).
 */
...
/* UTP Transfer Request Descriptor (UFSHCI spec §7.2.1, 32 bytes) */
typedef struct {
    uint32_t dw0;
    uint32_t dw1;
...
static void aspeed_ufs_process_slot(AspeedUFSState *s, int slot)
{
    ...
    if (trans_type == UPIU_TYPE_QUERY_REQ) {
        ...
        if (opcode == QUERY_OP_READ_DESC) {
            ...
            /* READ_ATTR, READ_FLAG, SET_FLAG, etc. — echo TSF, return success */
            ufs_dma_write(resp_base, rsp, 32);
        }
        goto done;
    }
    ...
    if (trans_type == UPIU_TYPE_CMD) {
        ...
        case SCSI_WRITE_16: {
            ...
            if (s->blk && byte_count > 0 && prdtl > 0 && xfer_buf) {
                ...
                if (blk_pwrite(s->blk, lba * 512, byte_count, xfer_buf, 0) < 0) {
                    ok = false;
                }
            }
            xfer_len = 0;
