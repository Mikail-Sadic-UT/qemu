...
/*
 * All CFAM flavours present a register slot holding a config table, an FSI
 * responder and a local bus carrying the engines the config table describes.
 * That is the common model; each flavour supplies its own table, slot layout
 * and engines.
 */
...
struct FSICFAMCommonClass {
    ...
    /* Realize and map this flavour's local bus engines */
